...
const TypingIndicator: React.FC<{ conversationId: string }> = ({
  conversationId,
}) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <div className="flex space-x-1">
        <div className="w-2 h-2 bg-gray-400 rounded-xl animate-bounce" />
        <div
...
sendMessage: async (chatId: string, content: string) => {
  ...
  try {
    ...
    set({
      messagesCache: { ...cache, [chatId]: { messages: cleanedMessages } },
    });
  }
};
```
...
const BloodCenterChat = () => {
  ...
  const loadConversations = useCallback(async () => {
    const [entradaResponse, esperandoResponse, finalizadosResponse] =
      await Promise.all([
    ...
    const donorsFromConversations = allConversations.map(
...
const handleSendMessage = async () => {
  const isFirstMessage =
    !selectedDonor.hasActiveConversation && !selectedDonor.lastMessage;
  ...
  return (
    <div className="modal">
      ...
      <div className="tabs">
        ...
        <button onClick={() => setActiveTab("new")}>Novo Contato</button>
      </div>
...
export const conversationApi = {
  ...
  search: (query: string, filters: SearchFilters) =>
    apiClient.get(`/conversations/search`, {
...
export const conversationAdapter = {
  ...
  toChat: (dto: ConversationDTO): Chat => ({
    ...
    lastMessage: dto.lastMessage
      ? messageAdapter.toMessage(dto.lastMessage)
      : null,
...
export const messageAdapter = {
  ...
  isTempMessage: (message: Message): boolean => message.id.startsWith("temp-"),
