...
interface UseWebSocketReturn {
  ...
  onInstanceConnected: (
    callback: (instanceId: string, phoneNumber: string) => void
  ) => void;
  onInstanceDisconnected: (
    callback: (instanceId: string, phoneNumber: string, error: string) => void
  ) => void;
}
...
export const useWebSocket = (): UseWebSocketReturn => {
  ...
  const instanceConnectedCallbackRef = useRef<
    ((instanceId: string, phoneNumber: string) => void) | null
  >(null);
  const instanceDisconnectedCallbackRef = useRef<
    ((instanceId: string, phoneNumber: string, error: string) => void) | null
  >(null);
  const { user } = useAuthStore();
  const accessToken = authService.getAccessToken();
  const { addMessage, updateConversation, setUserTyping } = useChatStore();
  ...
  const handleInstanceStatusChange = useCallback(
    (message: WebSocketMessage) => {
      ...
      if (message.type === "INSTANCE_STATUS_CHANGE") {
        ...
        if (
          message.statusData?.justConnected &&
          message.instanceId &&
          message.phoneNumber
        ) {
          console.log(
            "✅ Instance just connected, triggering callback:",
            message.instanceId
          );
          if (instanceConnectedCallbackRef.current) {
            instanceConnectedCallbackRef.current(
              message.instanceId,
              message.phoneNumber
            );
          }
          ...
          if (
            instanceDisconnectedCallbackRef.current &&
            message.instanceId &&
            message.phoneNumber
          ) {
  ...
  const connect = useCallback(() => {
    ...
    client.onConnect = () => {
      ...
      client.subscribe(`/user/topic/instance-status`, (message: IMessage) => {
        try {
          console.log(
            "🎯 Raw WebSocket message received on /user/topic/instance-status:",
            message.body
          );
          const data: WebSocketMessage = JSON.parse(message.body);
          ...
          console.error(
            "❌ Error parsing instance status notification:",
            error,
            "Raw body:",
            message.body
          );
      ...
      client.subscribe(`/user/topic/unread-counts`, (message: IMessage) => {
        try {
          ...
          console.error(
            "❌ Error parsing unread count notification:",
            error,
            "Raw body:",
            message.body
          );
  ...
  const onInstanceConnected = useCallback(
    (callback: (instanceId: string, phoneNumber: string) => void) => {
  ...
  const onInstanceDisconnected = useCallback(
    (
      callback: (instanceId: string, phoneNumber: string, error: string) => void
    ) => {
