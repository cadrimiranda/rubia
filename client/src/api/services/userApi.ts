...
export class UserAPI {
  ...
  async updateOnlineStatus(isOnline: boolean): Promise<UserDTO> {
    return apiClient.put<UserDTO>(`${this.basePath}/online-status`, { isOnline })
  }
  ...
  async changePassword(data: {
    ...
    return apiClient.put<{ success: boolean }>(`${this.basePath}/change-password`, data)
  }

  async refreshToken(): Promise<{ token: string; expiresIn: number }> {
    return apiClient.post<{ token: string; expiresIn: number }>('/api/auth/refresh')
  }
